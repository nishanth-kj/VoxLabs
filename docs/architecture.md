...
| `app/exceptions/` | `AppError` and its subclasses. Each declares an `ErrorCode` and `ErrorMessage` and carries a specific message plus an optional field. |
...
The API route `POST /api/tts` validates a `TTSRequest` and calls the same `tts_service.generate()` directly. With `background: true` it calls `generate_async()` instead and returns the job, wrapped in `ApiResponse(data=...).success()`.
