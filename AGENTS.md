...
- API routes only receive, validate, call a service and return `ApiResponse(data=...).success()`. Error handlers return `ApiResponse(error=exc).error()`. Every API response is **HTTP 200**, and the envelope is `{"status": 1|0, "data": ..., "error": null|{...}}`. The error object carries `type`, `error_code`, `error_message`, `field` (an object) and `detail`. Codes and texts are predefined in the `ErrorCode` and `ErrorMessage` BaseEnums (`app/constants/error_code.py`, `error_message.py`), and each exception class declares its pair.
...
- **Errors.** Services raise `AppError` subclasses from `app/exceptions` with a specific message and optional `field`. Each class maps to an `ErrorCode`/`ErrorMessage` pair. The UI shows `exc.message`; the API returns the full error object. Stack traces are only logged.
