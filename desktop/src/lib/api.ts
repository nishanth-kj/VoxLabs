...
export const api = {
  ...
  async registerVoice(params: { name: string; consent: boolean; projectId: string; audio: File }): Promise<{ voice_id: string }> {
    ...
    const res = await fetch(`${BASE_URL}/api/voices/register`, { method: "POST", body: form });
    return unwrap(res);
  },
  ...
  async designVoice(prompt: string, projectId: string): Promise<{ voice_id: string }> {
    ...
    const res = await fetch(`${BASE_URL}/api/voices/design`, { method: "POST", body: form });
    return unwrap(res);
  },

  async revokeVoice(voiceId: string): Promise<{ message: string }> {
    const res = await fetch(`${BASE_URL}/api/voices/${voiceId}`, { method: "DELETE" });
    return unwrap(res);
  },
  ...
  async textToSpeech(params: {
    ...
    const res = await fetch(`${BASE_URL}/api/tts`, { method: "POST", body: form });
    return unwrap(res);
  },
