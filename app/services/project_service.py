...
from app.exceptions import NotFoundError, ProjectError
...
from app.utils.validation import require_choice, require_name
...
class ProjectService:
    ...
    def create_project(self, name: str, project_type: str = ProjectType.AUDIO, description: str = "",
                       users_id: int | None = None) -> dict:
        project_type = require_choice(project_type, ProjectType.ALL, "project_type")
        with transaction() as session:
            project = Project(name=require_name(name), project_type=project_type, description=description,
                              users_id=users_id)
            session.add(project)
            session.flush()
    ...
    def save(self, projects_id: int | None = None, status: int | None = None, name: str | None = None,
             project_type: str | None = None, description: str | None = None, users_id: int | None = None,
             edit_state: dict | None = None, settings: dict | None = None) -> dict:
        """One entry point: create (no id), delete (status = Deleted) or update."""
        if projects_id is None:
            created = self.create_project(name, project_type or ProjectType.AUDIO, description or "", users_id)
            projects_id = created["projects_id"]
        elif status == Status.DELETED.code:
            self.delete_project(projects_id)
            return deleted_result("projects_id", projects_id)
        else:
            self.update_project(projects_id, name, project_type, description)
        if edit_state is not None or settings is not None:
            self.save_project(projects_id, edit_state=edit_state, settings=settings)
    ...
    def rename_project(self, projects_id: int, name: str) -> dict:
        with transaction() as session:
            project = self._get(session, projects_id)
            project.name = require_name(name)
            return self.to_dict(project)
    ...
    def update_project(self, projects_id: int, name: str | None = None, project_type: str | None = None,
        ...
        with transaction() as session:
            ...
            if project_type is not None:
                project.project_type = require_choice(project_type, ProjectType.ALL, "project_type")
            if description is not None:
                project.description = description
            return self.to_dict(project)
    ...
    def duplicate_project(self, projects_id: int, name: str | None = None) -> dict:
        ...
        with transaction() as session:
            ...
            copy = Project(name=require_name(name or f"{source.name} (copy)"), project_type=source.project_type,
