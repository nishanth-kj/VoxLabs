...
def test_generate_creates_labeled_audio_with_original_kept():
    audio = tts_service.generate("Hello world. This is VoxLabs.")
...
def test_speed_changes_duration():
    normal = tts_service.generate("Speed test sentence.", post={})
    fast = tts_service.generate("Speed test sentence.", post={}, speed=2.0)
...
def test_validation_errors():
    ...
    with pytest.raises(ValidationError):
        tts_service.generate("Hi", speed=5)
    with pytest.raises(ValidationError):
        tts_service.generate("Hi", emotion="furious")


def test_online_models_are_opt_in():
    with pytest.raises(ModelError, match="online"):
        tts_service.generate("Hi", model_key="gtts-emotional")
...
def test_regenerate_and_sentences():
    first = tts_service.generate("Regenerate me.", seed=1)
    again = tts_service.regenerate(first["audios_id"], seed=2)
    assert again["params"]["text"] == "Regenerate me." and again["params"]["seed"] == 2
    result = tts_service.generate_sentences("One. Two. Three.")
...
def test_clone_voice_with_cloning_model_and_fallback(voice_wav, consent):
    ...
    cloned = tts_service.generate("Speak in my voice.", voices_id=voice["voices_id"])
    ...
    matched = tts_service.generate("Approximate my voice.", voices_id=profile_voice["voices_id"])
